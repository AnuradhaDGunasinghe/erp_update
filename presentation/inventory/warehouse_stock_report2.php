...
<div class="row page-titles">
    <div class="col-md-5 align-self-center"><a href="./warehouse_stock_report2.php">
...
$search_value = 'null';
    if (isset($_POST['submit'])) {
        ...
        WHERE model LIKE '%$search_value%' AND send_to_production = '0' AND location = 'WH2'  AND reprint= '1' GROUP BY model,core";
        echo $query;
...
$brand = null;
        ...
        $processing = null;
        $dispatch = null;
        $query = "SELECT *,  COUNT(inventory_id) as main_total FROM `warehouse_information_sheet` WHERE location = 'WH2'  AND reprint= '1' GROUP BY brand ";
        ...
        foreach ($result as $data) {
            ...
            echo "
                                    ...
                                    <td> $main_stock</td>";
            echo "<td>";
            $query = "SELECT COUNT(inventory_id) as in_stock FROM `warehouse_information_sheet` WHERE brand = '$brand' AND send_to_production = '0' AND location = 'WH2'  AND reprint= '1'";
            $result = mysqli_query($connection, $query);
            foreach ($result as $data) {
                $in_stock = $data['in_stock'];

            } 
            $query = "SELECT COUNT(inventory_id) as processing FROM `warehouse_information_sheet` WHERE brand = '$brand' AND send_to_production = '1' AND location = 'WH2'  AND reprint= '1'";
            $result = mysqli_query($connection, $query);
            foreach ($result as $data) {
                $processing = $data['processing'];

            }
            $query = "SELECT  COUNT(inventory_id) as dispatch FROM `warehouse_information_sheet` WHERE brand = '$brand' AND dispatch = '1' AND location = 'WH2'  AND reprint= '1'";
            $result = mysqli_query($connection, $query);
            foreach ($result as $data) {
            ...
            echo "</td>
                                    <td class='text-center'>
                                        <a class='btn btn-xs bg-primary '
                                            href='model_summery2.php?brand=$brand'><i class='fas fa-eye'></i>
                                        </a>
                                    </td>
                                </tr>";
        }?>
